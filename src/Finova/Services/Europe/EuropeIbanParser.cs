...
public class EuropeIbanParser : IIbanParser
{
    ...
    private IEnumerable<IIbanParser>? _parsers;

    public EuropeIbanParser(IServiceProvider serviceProvider)
    {
        _serviceProvider = serviceProvider;
    }

    public EuropeIbanParser()
    {
        _serviceProvider = null;
    }

    private IEnumerable<IIbanParser> GetParsers()
    {
        if (_parsers == null && _serviceProvider != null)
        {
            _parsers = _serviceProvider.GetServices<IIbanParser>()
                                       .Where(p => p.GetType() != typeof(EuropeIbanParser));
        }
        return _parsers ?? Enumerable.Empty<IIbanParser>();
    }
